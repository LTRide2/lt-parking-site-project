...
- **`EnvironmentFile=…/.env`** — the production secrets (`SECRET_KEY`, `DATABASE_URL`, `CORS_ORIGINS`). This is the server's equivalent of your local `.env` from [Lesson B0](https://github.com/LTRide2/LTR-Backend/tree/main/plan/backend/lessons/B0-clean-slate-and-safety.md) — it lives only on the box, readable only by `ltride`, never committed.
