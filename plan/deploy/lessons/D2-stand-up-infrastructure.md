...
- [ ] `scripts/deploy.sh infra status` shows every stack as `CREATE_COMPLETE`.
- [ ] `scripts/deploy.sh infra outputs` prints a live **EC2 public IP** and an **RDS endpoint**.
- [ ] `ssh -i ~/.ssh/ltride-key.pem ubuntu@<ElasticIp>` logs you into the server (`exit` to leave).
- [ ] You know how to run `scripts/deploy.sh infra down` to tear it all back down when you're done for the day.

---

## 🤔 Why this lesson matters

Every lesson before this one was **describing** infrastructure in YAML or shell script — no AWS bill, no risk. This lesson is different: `scripts/deploy.sh infra up` actually asks Amazon to create a computer and a database, and Amazon starts charging you the moment they exist. With the default settings this repo ships (`t3.micro` EC2, `db.t3.micro` RDS), that's a **few dollars a month** — see the full breakdown in the deployment guide's [monthly cost estimate](../deployment-guide.md#b13-monthly-cost-estimate-scaling-scenarios) (the "minimum" / default column is the one that matches these defaults). Nothing here will bankrupt a student project, but only if you remember the other half of this lesson: **you can turn it all off**.

`scripts/deploy.sh infra down` deletes every stack — the EC2 instance, the RDS database, the networking — so nothing keeps billing while you sleep or between study sessions. RDS keeps a final snapshot on the way out, so you don't lose your data; you can recreate everything with `scripts/deploy.sh infra up` whenever you pick the project back up. This "spin it up, prove it works, spin it down" rhythm is exactly the habit D0 asked you to set a billing alarm for — this is the lesson where that habit gets exercised for real.
...
**Prerequisites:** D0 done (AWS CLI configured, `~/.ssh/ltride-key.pem` in place); D1's four templates pass `scripts/deploy.sh infra validate`; D1b's server config files are in `deploy/server/`.

**Time budget for the hour:** branch + preflight (5 min) → kick off `scripts/deploy.sh infra up` and wait for RDS (10–15 min, mostly waiting) → watch `scripts/deploy.sh infra status` (5 min, overlaps with the wait) → `scripts/deploy.sh infra outputs` + SSH proof (10 min) → read this lesson's troubleshooting section so you recognize a stack failure if one happens (10 min) → decide whether to leave it running or `scripts/deploy.sh infra down` (5 min buffer).
...
```bash
scripts/deploy.sh infra status
```
...
> **If a stack shows `ROLLBACK_COMPLETE` or `CREATE_FAILED` instead:** open the AWS Console → **CloudFormation** → click the failed stack → **Events** tab. The first red row is the real error (often a bad parameter or a name collision). Fix the template, then just re-run `scripts/deploy.sh infra up` — it updates in place, it doesn't start over. → Reference: [Viewing CloudFormation stack events](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-viewing-stack-events.html).

### Step 3 — Grab the live addresses (~5 min)

```bash
scripts/deploy.sh infra outputs
```
...
**Expected:**
1. All four stacks eventually read `CREATE_COMPLETE` in `scripts/deploy.sh infra status`.
2. `scripts/deploy.sh infra outputs` prints a real public IP address (your Elastic IP) and an RDS endpoint hostname.
...
> 💸 **Cost control:** once you've proven all of the above, if you're done for the day and don't need the server live, run `scripts/deploy.sh infra down` to delete everything (RDS keeps a final snapshot). Re-create anytime with `scripts/deploy.sh infra up`.
...
- **A stack shows `CREATE_FAILED` or rolls back** — AWS Console → CloudFormation → click the stack → **Events** tab; the first red row names the exact resource and reason. Fix the template and re-run `scripts/deploy.sh infra up`.
...
- You ran `scripts/deploy.sh infra up`, which called `aws cloudformation deploy` on all four templates in dependency order, and watched them reach `CREATE_COMPLETE` with `scripts/deploy.sh infra status`.
- You pulled the live **Elastic IP** and **RDS endpoint** out of `scripts/deploy.sh infra outputs`, and used the IP to **SSH into your real server**.
...
- You practiced the cost habit this whole track depends on: stand it up to prove it works, tear it down with `scripts/deploy.sh infra down` when you're not using it.
...
**[Lesson D3 — Release the application code](D3-release-application-code.md).** You'll put your actual backend and frontend onto the running server with `scripts/deploy.sh app`. → [source CR](../deployment-guide.md#cr-d3--release-the-application-code).
